...
def select_provider_and_model(
        ...
        if provider_lower in policy.providers:
            ...
            if provider_config.enabled:
                ...
                fallback_chain = _get_fallback_chain(
                    provider_lower, policy) if enable_fallback else []
    ...
    if not policy.enable_auto_routing:
        ...
        model = requested_model or policy.default_model or (
            provider_config.models[0] if provider_config and provider_config.models else None)
        fallback_chain = _get_fallback_chain(
            default_provider, policy) if enable_fallback else []
    ...
    provider, model = _apply_complexity_rules(
        complexity, policy, requested_model)
    if provider:
        fallback_chain = _get_fallback_chain(
            provider, policy) if enable_fallback else []
        return (provider, model, fallback_chain)

    # Apply cost/quality trade-offs
    provider, model = _apply_cost_quality_tradeoff(
        cost_mode, policy, requested_model)
    if provider:
        fallback_chain = _get_fallback_chain(
            provider, policy) if enable_fallback else []
        return (provider, model, fallback_chain)
...
def _apply_cost_quality_tradeoff(
    ...
    mode = cost_mode if cost_mode in [
        "cost_optimized", "balanced", "quality_optimized"] else cq.mode
    ...
    if mode == "cost_optimized":
        ...
        providers_by_cost = sorted(
            [(name, config)
             for name, config in policy.providers.items() if config.enabled],
            key=lambda x: {"low": 1, "medium": 2,
                           "high": 3}.get(x[1].cost_tier, 2)
        )
        ...
        providers_by_cost = sorted(
            [(name, config)
             for name, config in policy.providers.items() if config.enabled],
            key=lambda x: {"low": 3, "medium": 2,
                           "high": 1}.get(x[1].quality_tier, 2)
        )
    else:  # balanced
        # Select by priority
        providers_by_cost = sorted(
    ...
    for attempt_provider in providers_to_try:
        ...
        try:
            logger.info("Attempting provider",
                        provider=attempt_provider, model=model)
            ...
            if cb_config and policy.enable_circuit_breaker:
                ...
                if not breaker.can_attempt():
                    logger.warn(
                        "Circuit breaker is open, skipping provider", provider=attempt_provider)
                    continue
            ...
            async def _execute_with_timeout_and_retry():
                ...
                # Wrap execute_fn to match the expected signature for execute_with_resilience
                async def _wrapped_execute_fn():
                    return await execute_fn(attempt_provider, model, *args, **exec_kwargs)

                # Call with positional arguments to avoid any keyword argument conflicts
                return await execute_with_resilience(
                    _wrapped_execute_fn,     # func
                    attempt_provider,         # provider
                    retry_strategy,           # strategy (positional)
                    policy.enable_retry       # enable_retry (positional)
                )
            ...
            if attempt_provider != provider:
                logger.info("Fallback provider succeeded",
                            original=provider, fallback=attempt_provider)
            return result
            ...
            logger.warn("Provider failed, trying fallback",
                        provider=attempt_provider, error=str(e))
            continue
